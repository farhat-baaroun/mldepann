...
const CTAButtons = memo(function CTAButtons({ 
  ...
  const getUserLocation = useCallback(() => {
    ...
    setIsLocating(true)
    // Use requestIdleCallback for non-critical geolocation
    const geolocationTask = () => {
    ...
    if ('requestIdleCallback' in window) {
      window.requestIdleCallback(geolocationTask, { timeout: 2000 })
    } else {
      setTimeout(geolocationTask, 0)
    }
  }, [])
  ...
  // Build Snapchat share link
  const getSnapchatLink = useCallback(() => {
    return snapchatLink
  }, [])

  const isHeaderVariant = variant === 'header'
  const iconSize = isHeaderVariant ? 'w-4 h-4' : 'w-5 h-5'
  
  // Button classes - icons only for desktop navbar, full width rows for CTA
  const buttonBaseClass = isHeaderVariant 
    ? 'inline-flex items-center justify-center gap-0 px-3 py-2 rounded-full text-xs font-semibold transition-all hover:opacity-90 min-w-[40px]'
    : isMobileMenu
      ? 'w-full flex items-center justify-start gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all hover:opacity-90 shadow-sm text-left'
      : 'flex-1 flex items-center justify-center gap-3 px-4 py-3 rounded-lg text-sm md:text-base font-medium transition-all hover:opacity-90 shadow-lg'

  // Container classes - different for mobile menu vs CTA sections
  const containerClass = isHeaderVariant 
    ? 'flex items-center gap-1.5'
    : isMobileMenu
      ? 'flex flex-col gap-2 w-full'
      : 'flex flex-col sm:flex-row gap-3 w-full max-w-2xl mx-auto'
  ...
  return (
    <>
      <div className={containerClass}>
        ...
        <button
          ...
          className={`${buttonBaseClass} ${
            userLocation 
              ...
              : isHeaderVariant 
                ? 'bg-brand-blue text-white hover:bg-brand-blue-light md:bg-white/10 md:backdrop-blur-sm md:text-white md:hover:bg-white/20' 
                : 'bg-brand-blue text-white hover:bg-brand-blue-light'
          }`}
          aria-label={isHeaderVariant ? (userLocation ? 'Position activée' : 'Activer ma position') : 'Partager votre position'}
          disabled={isLocating}
        >
          <Navigation className={`${iconSize} ${isLocating ? 'animate-spin' : ''}`} />
          {!isHeaderVariant && (
            <span>{isLocating ? '...' : userLocation ? '✓ ' : ''}Partager votre position</span>
          )}
        </button>
        ...
        <a
          href={phoneLink}
          className={`${buttonBaseClass} bg-brand-green text-white hover:bg-brand-green/90`}
          aria-label={isHeaderVariant ? 'Appeler maintenant' : 'Appelez nous'}
        >
          <Phone className={iconSize} />
          {!isHeaderVariant && <span>Appelez nous</span>}
        </a>
        ...
        <a
          ...
          className={`${buttonBaseClass} bg-[#25d366] text-white hover:bg-[#25d366]/90`}
          aria-label={isHeaderVariant ? 'Contacter sur WhatsApp' : 'Nous contacter sur WhatsApp'}
        >
          <svg 
            className={iconSize} 
            viewBox="0 0 48 48" 
          ...
          {!isHeaderVariant && <span>Nous contacter sur WhatsApp</span>}
        </a>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={`${buttonBaseClass} bg-[#fffc00] text-black hover:bg-[#fffc00]/90`}
          aria-label={isHeaderVariant ? 'Contacter sur Snapchat' : 'Nous contacter sur Snapchat'}
        >
          <svg 
            className={iconSize} 
          ...
          {!isHeaderVariant && <span>Nous contacter sur Snapchat</span>}
        </a>
      </div>
      ...
      {showLocationInfo && (
        <div className="mt-6 text-center">
          <p className="text-sm md:text-base text-white/90 max-w-3xl mx-auto leading-relaxed px-4">
            <strong className="text-white">Partage de localisation optionnel :</strong> Si vous êtes en urgence, activez le partage de localisation et envoyez-nous un message WhatsApp pour que nous recevions instantanément votre position et puissions continuer la conversation.
          </p>
        </div>
      )}
